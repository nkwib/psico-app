<script lang="ts">
	...
	let fattureFiltrate = $derived(invoicesData.filter(fattura => {
	...
	function cambiaStato(id: number, nuovoStato: 'bozza' | 'emessa' | 'pagata') {
...
<div class="space-y-6">
	...
	<div class="flex justify-between items-center">
		<div>
			<h1 class="text-3xl font-bold text-gray-900">Fatture</h1>
			<p class="text-gray-600 mt-1">Gestisci le fatture emesse</p>
		</div>
	...
	<div class="grid grid-cols-1 md:grid-cols-4 gap-4">
		<Card>
			...
			<CardContent>
				<div class="text-2xl font-bold">{invoiceStatsData.total}</div>
			</CardContent>
		</Card>
		...
		<Card>
			...
			<CardContent>
				<div class="text-2xl font-bold">{invoiceStatsData.pending}</div>
			</CardContent>
		</Card>
		...
		<Card>
			...
			<CardContent>
				<div class="text-2xl font-bold">{formattatoreValuta.format(invoiceStatsData.revenue)}</div>
			</CardContent>
		</Card>
	</div>
				...
				<Select.Root type="single" bind:value={filtroStato}>
					<Select.Trigger class="w-[180px]">
						{filtroStato === 'tutte' ? 'Tutte' : 
						 filtroStato === 'bozza' ? 'Bozza' :
						 filtroStato === 'emessa' ? 'Emessa' : 'Pagata'}
					</Select.Trigger>
					<Select.Content>
						<Select.Item value="tutte">Tutte</Select.Item>
						<Select.Item value="bozza">Bozza</Select.Item>
						...
						<TableBody>
							{#each fattureFiltrate as fattura}
								<TableRow>
									...
									<TableCell>
										{fattura.psicologo?.nome} {fattura.psicologo?.cognome}
									</TableCell>
									<TableCell class="font-medium">
			...
			{:else}
				<div class="text-center py-12">
					...
					<p class="text-gray-500 mb-4">
						{ricerca || filtroStato !== 'tutte' ? 'Nessuna fattura trovata con i criteri di ricerca' : 'Nessuna fattura emessa'}
					</p>
