<script lang="ts">
	...
	let psychologistsList = $state<any[]>([]);
	...
	const { form, errors, enhance, constraints, submitting, tainted } = superForm(data.form, {
		...
		onSubmit: async ({ formData, cancel }) => {
			...
			if (!psychologist) {
				toast.error('Psicologo non trovato');
				return;
			}
			...
			psychologists.update(Number(data.psychologistId), datiAggiornati);
			
			toast.success(`Psicologo ${datiAggiornati.nome} ${datiAggiornati.cognome} aggiornato con successo`);
			setTimeout(() => {
				goto('/psicologi');
			}, 100);
		},
		onError: ({ result }) => {
			...
			toast.error('Errore nell\'aggiornamento del psicologo');
		}
	});

	onMount(() => {
		// Subscribe to psychologists store
		const unsubscribe = psychologists.subscribe((allPsychologists) => {
			psychologistsList = allPsychologists;
			psychologist = allPsychologists.find(p => p.id === Number(data.psychologistId));
			
			if (psychologist) {
				// Update form with existing data
				$form.nome = psychologist.nome;
				$form.cognome = psychologist.cognome;
				$form.codiceFiscale = psychologist.codiceFiscale;
				$form.partitaIva = psychologist.partitaIva;
				$form.indirizzo = psychologist.indirizzo;
				$form.telefono = psychologist.telefono;
				$form.email = psychologist.email;
				$form.numeroOrdine = psychologist.numeroOrdine || '';
				$form.isPreferito = psychologist.isPreferito || false;
				
				// Fatturazione elettronica fields
				fatturaElettronicaAbilitata = psychologist.fatturaElettronicaAbilitata || false;
				regimeFiscale = psychologist.regimeFiscale || 'forfettario';
				pecFatturazione = psychologist.pecFatturazione || '';
				codiceDestinatario = psychologist.codiceDestinatario || '';
			}
			loading = false;
		});
...
<div class="container mx-auto py-8 max-w-4xl">
	<div class="mb-6">
		<h1 class="text-3xl font-bold text-gray-900">Modifica Psicologo</h1>
		<p class="text-gray-600 mt-2">Aggiorna i dati del professionista</p>
	</div>
	...
	{:else if !psychologist}
		<Alert variant="destructive">
			<AlertDescription>
				Psicologo non trovato. <a href="/psicologi" class="underline">Torna alla lista</a>
			</AlertDescription>
		</Alert>
	{:else}
			...
			<div class="flex justify-between pt-6 border-t">
				...
				<div class="flex gap-3">
					<Button 
						...
						type="button" 
						onclick={() => {
							if (confirm(`Sei sicuro di voler eliminare ${psychologist.nome} ${psychologist.cognome}?`)) {
								psychologists.remove(Number(data.psychologistId));
								toast.success('Psicologo eliminato con successo');
								goto('/psicologi');
							}
						}}
					>
